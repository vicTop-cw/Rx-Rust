...
class _OpModule:
    ...
    def write_to_clipboard(dispatcher, source=None):
        ...
        if not _is_windows:
            raise RuntimeError("write_to_clipboard is only supported on Windows")
    ...
    def write_to_filesystem(dispatcher, mode="create"):
        ...
        if not _is_windows:
            raise RuntimeError("write_to_filesystem is only supported on Windows")
...
if _is_windows:
    try:
        ...
        ChangeType = ClipData = ClipboardDispatcher = None
        ClipSubject = ClipObserver = from_clipboard = write_to_clipboard = None
else:
    ChangeType = ClipData = ClipboardDispatcher = None
    ClipSubject = ClipObserver = from_clipboard = write_to_clipboard = None

# 文件系统监控模块 (Windows only)
if _is_windows:
    try:
        ...
        FileChangeType = FileData = FileDispatcher = None
        FileSubject = FileObserver = from_filesystem = write_to_filesystem = None
else:
    FileChangeType = FileData = FileDispatcher = None
    FileSubject = FileObserver = from_filesystem = write_to_filesystem = None

# 目录系统监控模块 (Windows only)
if _is_windows:
    try:
        ...
        FolderChangeType = FolderData = FolderDispatcher = None
        FolderSubject = FolderObserver = from_foldersystem = write_to_foldersystem = None
else:
    FolderChangeType = FolderData = FolderDispatcher = None
    FolderSubject = FolderObserver = from_foldersystem = write_to_foldersystem = None

# 键盘鼠标响应式模块 (Windows only)
if _is_windows:
    try:
        ...
        KeyEventType = KeyData = MouseEventType = None
        MouseData = KeyModifier = KeyboardDispatcher = MouseDispatcher = None
        KeySubject = MouseSubject = KeyObserver = MouseObserver = None
        from_keyboard = from_mouse = write_to_keyboard = write_to_mouse = None
else:
    KeyEventType = KeyData = MouseEventType = None
    MouseData = KeyModifier = KeyboardDispatcher = MouseDispatcher = None
    KeySubject = MouseSubject = KeyObserver = MouseObserver = None
